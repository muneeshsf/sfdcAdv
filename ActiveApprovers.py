...
temp_csv_file = "temp_user_data.csv"
...
full_command = f'"{sf_path}" data query --query "{query}" --target-org {username_alias} --result-format csv --output {temp_csv_file}'
try:
    ...
    os.remove(temp_csv_file)  # Remove the temporary CSV file
